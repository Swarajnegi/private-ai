...
const OPENCLAUDE_DIR = join(__dirname, '..')
...
// Use shell:false for direct node invocations with absolute paths.
// shell:true joins args into a string — spaces in paths like 'AI Model Reops' get split by the shell.
function runAgent(execPath: string, args: string[], env: NodeJS.ProcessEnv, cwd?: string): Promise<number> {
  return new Promise(resolve => {
...
async function main(): Promise<void> {
  ...
  // Launch the agent using runAgent (shell:false) so the absolute path to cli.mjs
  // is passed directly to node without shell tokenization — fixes 'AI Model Reops' space split.
  const cliAbsPath = join(OPENCLAUDE_DIR, 'dist', 'cli.mjs')
  const agentCwd = process.env.JARVIS_WORKSPACE ?? OPENCLAUDE_DIR
  const devCode = await runAgent(
    process.execPath,           // absolute path to node.exe — no shell lookup needed
    [cliAbsPath, ...options.passthroughArgs],
    env,
    agentCwd
  )
